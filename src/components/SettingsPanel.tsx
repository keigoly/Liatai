...
// ========== Googleアカウントセクション ==========
const GoogleAccountSection: React.FC<{ t: (key: TranslationKey) => string }> = ({ t }) => {
  const { user, isSignedIn, isLoading, error, signIn, signOut } = useAuthContext();

  return (
    <div className="border border-[var(--border-color)] rounded-lg overflow-hidden bg-[var(--card-bg-color)] transition-colors">
      <div className="p-3">
        <div className="flex items-center gap-2 mb-3">
          <span className="text-sm font-bold text-white">{t('account')}</span>
          <span className="bg-red-500 text-white text-[0.6rem] font-bold px-1.5 py-0.5 rounded animate-pulse">NEW</span>
        </div>
        ...
        {isSignedIn && user ? (
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-white text-sm font-bold truncate">{user.displayName || 'User'}</div>
            ...
            <div className="flex items-center gap-2 px-1">
              ...
              <span className="text-gray-400 text-xs">{t('syncStatusIdle')}</span>
            </div>

            {/* サインアウトボタン */}
            <button
              onClick={signOut}
              className="w-full py-2 px-3 border border-[var(--border-color)] rounded-lg text-red-400 text-xs font-bold hover:bg-red-400/10 transition-colors"
            >
              {t('signOut')}
            </button>
          </div>
          ...
          <div className="space-y-3">
            {/* 説明テキスト */}
            <p className="text-gray-400 text-xs leading-relaxed">{t('signInDescription')}</p>

            {/* Googleサインインボタン */}
            <button
              onClick={signIn}
              disabled={isLoading}
              className="w-full flex items-center justify-center gap-2 py-2.5 px-4 bg-[var(--bg-color)] border border-[var(--border-color)] hover:brightness-125 rounded-lg transition-colors disabled:opacity-50"
            >
              {isLoading ? (
              ...
              <span className="text-white font-bold text-sm">{t('signIn')}</span>
            </button>
          </div>
        )}
        ...
        {error && (
          <div className="mt-2 p-2 bg-red-500/10 border border-red-500/30 rounded-lg">
            <p className="text-red-400 text-xs">{t('signInError')}: {error}</p>
          </div>
        )}

        {/* プライバシーノート */}
        <p className="text-gray-500 text-[10px] leading-relaxed mt-3">{t('privacyNotice')}</p>
      </div>
    </div>
  );
};
  ...
  return (
    <>
      ...
      <div className="p-4 space-y-4" ref={parent}>
        {/* ========== Googleアカウント（v1.3.0 新機能・一番上に配置） ========== */}
        <GoogleAccountSection t={t} />
