...
### 新機能
* **🎨 フォルダのサブカラー**: フォルダに2色目を設定でき、ラベルを2色表示にできるようになりました。フォルダが増えても一目で見分けられます。

### 修正
* **☁️ 登録ワード・フォルダの同期**: 登録ワードとフォルダの変更がクロスデバイス同期に反映されない問題を修正しました。削除も他の環境へ正しく伝わります。
* **📱 SNSシェアの重複表示**: SNSシェアがパネル下部に二重に表示される問題を修正しました。
...
* **📊 ポスト数グラフ**: 検索結果にポスト数推移グラフを表示。6時間・24時間・7日・30日の期間切り替え対応。
* **📄 もっと見る**: 過去のポストを遡って表示できるようになりました。
